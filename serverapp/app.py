...
import statement4dbope as st1db
...
app.json_encoder = mg.MongoJSON_Encoder
...
@app.route('/ue_marks/<usn>/<string:term>/<year>')

def get_ue_marks(usn,term,year):
    ue_marks = st1db.get_ue_marks_details(usn,term,year)
    return jsonify({"marks":ue_marks})

@app.route('/companydetails/<usn>')
#view company details of a student
def get_placement_details(usn):
    company_details = st1db.get_placement_details(usn)
    return jsonify({"company":company_details})
