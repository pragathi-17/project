...
pms_placement_student_details = mydb['pms_placement_student_details']
...
def get_placement_details(usn):
    res = []
    pms_array=pms_placement_student_details.aggregate([
...
def get_ue_marks_details(usn,term,year):
    semester = term[-1]
    res = []

    ue_marks=pms_university_exam.aggregate([
    {"$match":{"academicYear" :year}},
    {"$unwind":"$terms"},
    {"$match":{"terms.termNumber":semester}},
    {"$unwind":"$terms.scores"},
    {"$match":{"terms.scores.usn":usn}},
    {"$project":{"course":"$terms.scores.courseScores"}},
    {"$unwind":"$course"},
    {"$project":{"courseName":"$course.courseName","totalScore":"$course.totalScore","maxtotalscore":"$course.maxTotalScore","percentage":"$course.percentage","_id":0}}
    ])
    for n in ue_marks:
        res.append(n)
    return res
